 ...
 */
abstract class DataMapCollection
{
	...
	public function append($element) {
		if (!$this->baseClassIsValid() || !is_a($element, $this->baseClass)) 
		    throw new Exception("'".get_class($element)."'' given; expecting '".$this->baseClass."'. You must supply a valid base class and the appended element must be a subclass of it.");
	...
	public function create($values) {
		if (!$this->baseClassIsValid()) 
		    throw new Exception("You can't create new DataMaps if you don't define a valid base class."); 
	...
	public function remove($index) {
		if ($index < 0 || $index >= count($this->collection)) 
		    throw new Exception("Index out of collection bounds. Expected number between 0 and " . count($this->collection));
